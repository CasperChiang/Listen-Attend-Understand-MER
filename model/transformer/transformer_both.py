...
# 修改这个函数，让它同时返回编码器块的输出和注意力分数
def transformer_encoder_block(inputs, head_size, num_heads, ff_dim, dropout=0):
    """构建一个Transformer Encoder块，并返回注意力分数"""
    # Multi-Head Self-Attention
    x = layers.LayerNormalization(epsilon=1e-6)(inputs)

    # *** 主要修改在这里 ***
    # 创建一个 MultiHeadAttention 层实例
    attention_layer = layers.MultiHeadAttention(
        key_dim=head_size, num_heads=num_heads, dropout=dropout
    )
    # 调用时，设置 return_attention_scores=True
...
def build_mer_model():
    ...
    # *** 主要修改在这里 ***
    # 因为 TRANSFORMER_LAYERS = 1，我们直接接收 attention_scores
    # 如果你有多层，通常可视化最后一层的注意力
    attention_scores = None
    for _ in range(TRANSFORMER_LAYERS):
        # 函数现在返回两个值
        x, attention_scores = transformer_encoder_block(
            x, MODEL_DIM // TRANSFORMER_HEADS, TRANSFORMER_HEADS, TRANSFORMER_UNITS, DROPOUT_RATE
        )

    # 6. 输出层
    va_output = layers.Dense(2, activation='linear', name="va_output")(x)

    # 构建模型，注意 outputs 是一个列表
    model = keras.Model(
        inputs=[mel_input, coch_input],
        outputs=[va_output, attention_scores], # <--- 模型现在有两个输出
        name="Interpretable_MER_Model"
    )
    return model
