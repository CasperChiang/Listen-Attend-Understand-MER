...
# 这将从测试集中加载数据
try:
    from data_pro.load_dataset_mel import test_dataset
except ImportError:
    print("错误: 无法导入 test_dataset。")
...
def evaluate():
    ...
    # 2. 逐批次进行预测并收集真实标签和预测结果 (使用之前修正过的逻辑)
    print("\n正在对测试集进行逐批次预测并收集结果...")
    y_true_list = []
    y_pred_list = []

    # 遍历测试数据集的每一个批次
    for mel_batch, (labels_batch, _) in test_dataset:
        # 对当前批次进行预测
        predictions = model.predict_on_batch(mel_batch)

        # 处理模型输出
        if isinstance(predictions, list):
            y_pred_va_batch = predictions[0]
        else:
            y_pred_va_batch = predictions
    ...
    # 3. 将所有批次的结果连接成一个大的Numpy数组 (这部分不变)
    ...
    # --- *** 新增：计算整体指标 *** ---
    # 整体 CCC 是 Valence 和 Arousal CCC 的平均值
    ccc_overall = np.mean(ccc_scores)
    # 直接在 reshaped 的二维数组上计算整体 R^2 和 RMSE
    r2_overall = r2_score(y_true_reshaped, y_pred_reshaped)
    rmse_overall = np.sqrt(mean_squared_error(y_true_reshaped, y_pred_reshaped))
    ...
    # --- *** 新增：打印整体性能结果 *** ---
    print("\n[Overall (整体性能)]")
    print(f"  CCC   : {ccc_overall:.4f}  (V和A的平均值)")
    print(f"  R^2   : {r2_overall:.4f}  (V和A的平均R^2)")
    print(f"  RMSE  : {rmse_overall:.4f}  (V和A的总体RMSE)")
    print("\n--------------------------")
